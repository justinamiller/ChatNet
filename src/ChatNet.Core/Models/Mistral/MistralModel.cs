...
{
    /// <summary>
    /// Mistral model implementation. Uses the same transformer architecture as Llama:
    /// SiLU-gated FFN, RMSNorm, RoPE, and GQA.
    /// Sliding window attention is not required for correctness and is omitted.
    /// </summary>
    public sealed class MistralModel : IModel
    {
        private readonly MistralConfig _cfg;
        private readonly MistralWeights _weights;
        ...
        public MistralModel(ModelConfig modelConfig, MistralWeights weights)
        {
            _modelConfig = modelConfig;
            _cfg = new MistralConfig(modelConfig);
            _weights = weights;
