...
{
    /// <summary>
    /// Qwen2 model implementation. Architecturally identical to Llama with optional QKV biases.
    /// Uses SiLU-gated FFN, RMSNorm, RoPE, and GQA.
    /// </summary>
    public sealed class QwenModel : IModel
    {
        private readonly QwenConfig _cfg;
        private readonly QwenWeights _weights;
        ...
        public QwenModel(ModelConfig modelConfig, QwenWeights weights)
        {
            _modelConfig = modelConfig;
            _cfg = new QwenConfig(modelConfig);
            _weights = weights;
            ...
            for (int t = 0; t < tokenIds.Length; t++)
            {
                ...
                for (int l = 0; l < layers; l++)
                {
                    ...
                    unsafe
                    {
                        ...
                        // Qwen2: add biases to Q, K, V if present
                        if (hasBias)
                        {
                            float* qBias = _weights.GetAttnQBias(l);
                            float* kBias = _weights.GetAttnKBias(l);
                            float* vBias = _weights.GetAttnVBias(l);
                            if (qBias != null)
                            {
                                fixed (float* pQ = _q)
                                fixed (float* pK = _k)
                                fixed (float* pV = _v)
                                {
                                    for (int i = 0; i < dim; i++) pQ[i] += qBias[i];
                                    for (int i = 0; i < kvDim; i++) pK[i] += kBias[i];
                                    for (int i = 0; i < kvDim; i++) pV[i] += vBias[i];
                                }
                            }
                        }
                    }
