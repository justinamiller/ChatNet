...
{
    /// <summary>
    /// Q5_1 dequantization: blocks of 32 values.
    /// Each block = 2 bytes (half scale) + 2 bytes (half min) + 4 bytes (32 high-bits) + 16 bytes = 24 bytes.
    ///
    /// Dequantization: value = q5 * scale + min
    ...
    ///   qs[16]   : 32 low nibbles packed (2 per byte)
    /// </summary>
    public static class DequantQ5_1
    {
        public const int BlockSize = 32;
        public const int BytesPerBlock = 24;

        private const int DOffset = 0;
        private const int MOffset = 2;
        private const int QhOffset = 4;
        ...
        public static void Dequantize(ReadOnlySpan<byte> quantizedData, Span<float> output, int elementCount)
        {
            ...
            for (int b = 0; b < blockCount; b++)
            {
                ...
                float min = DequantQ4_0.HalfToFloat(quantizedData[srcOffset + MOffset], quantizedData[srcOffset + MOffset + 1]);
                ...
                for (int j = 0; j < half; j++)
                {
                    ...
                    int q5 = low | highBit;
                    output[dstOffset + j] = q5 * scale + min;
                }
                ...
                for (int j = 0; j < secondHalf; j++)
                {
                    ...
                    int q5 = high | highBit;
                    output[dstOffset + 16 + j] = q5 * scale + min;
                }
        ...
        private static unsafe float DotProductScalar(byte* quantizedRow, float* input, int elementCount)
        {
            ...
            for (int b = 0; b < blockCount; b++)
            {
                ...
                float min = DequantQ4_0.HalfToFloat(quantizedRow[srcOffset + MOffset], quantizedRow[srcOffset + MOffset + 1]);
